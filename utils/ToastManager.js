export class ToastManager {
    ...
    static error(err) {
        ...
        if (lowered.includes('encrypted') || lowered.includes('password') || lowered.includes('encrypt dictionary')) {
            msg = "Error: PDF is encrypted or password-protected. Please unlock it using the Unlock tool first.";
        } else if (lowered.includes('pdf size') || lowered.includes('too large') || lowered.includes('out of memory') || lowered.includes('allocation failed') || lowered.includes('warning:')) {
            type = 'warning';
            if (!msg.toLowerCase().includes('warning:')) {
                msg = "Warning: File size is very large. Processing might take a while or limit browser memory resources.";
            }
