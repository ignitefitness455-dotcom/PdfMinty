...
export const onRequest: PagesFunction<any> = async (context) => {
  ...
  // 1. Rate Limiting: max 3 contact inquiries per rolling hour (3600 seconds)
  const rateLimitResult = await checkRateLimit(context.request, context.env, "contact", 3, 3600);
  ...
  const validationResult = await validateBody(context.request, contactSchema);
  ...
  if (detectSpamHeuristics(payload)) {
    const responseHeaders = handleCors(context.request, new Headers({ "Content-Type": "application/json" }));
    // Return 400 Bad Request to indicate spam filter rejection
    return new Response(
      JSON.stringify({
        success: false,
        error: "SPAM_DETECTED",
  ...
  if (kv) {
    try {
      const submissionId = `submission:contact:${Date.now()}:${Math.random().toString(36).substring(2, 7)}`;
      ...
      console.error("Failed to write contact submission to KV:", err);
    }
  }
