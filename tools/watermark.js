...
export default function renderTool() {
    ...
    if (opacityInput) opacityInput.addEventListener('input', (e) => opacityVal.textContent = e.target.value + '%');
    if (sizeInput) sizeInput.addEventListener('input', (e) => sizeVal.textContent = e.target.value + 'px');
    if (bgOpacityInput) bgOpacityInput.addEventListener('input', (e) => bgOpacityVal.textContent = e.target.value + '%');
        },
        onApply: async ({ actualBytes, currentFileName }) => {
            ...
            if (typeof window.runPdfWorkerTask === 'function') {
                const payload = { fileBytes: new Uint8Array(actualBytes), text, colorRgb: {r,g,b}, opacity, textSize, rotationDeg, position };
                resultBytes = await window.runPdfWorkerTask('watermark', payload, [payload.fileBytes.buffer]);
            } else {
                throw new Error("Worker not found");
            }
            
            if (typeof downloadFile === 'function') downloadFile(resultBytes, currentFileName + '_watermarked.pdf');
