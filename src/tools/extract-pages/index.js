/**
 * src/tools/extract-pages/index.js
 */
...
async function handleProcess(fileBytes, options) {
  ...
  try {
    showProgress(10, 'Initializing page extraction...');

    const payload = {
      fileBytes,
      pagesToKeep: options.pagesToKeep
    };

    const resultBytes = await runTask('extract-pages', payload, {
    ...
    downloadFile(resultBytes, options.fileName + '_extracted.pdf');
    showSuccess('Pages extracted successfully!');

    // Analytics
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('extract-pages', processingTime, fileBytes.byteLength, 0, true);

  } catch (err) {
    hideProgress();
    showError(err.message || 'Page extraction failed');

    const processingTime = window.performance.now() - startTime;
    trackToolUsage('extract-pages', processingTime, fileBytes.byteLength, 0, false, err.message);
  }
}
