/**
 * src/tools/delete-pages/index.js
 */
...
async function handleProcess(fileBytes, options) {
  ...
  try {
    showProgress(10, 'Initializing page deletion...');

    const payload = {
      fileBytes,
      pagesToDelete: options.pagesToDelete
    };

    const resultBytes = await runTask('delete-pages', payload, {
    ...
    downloadFile(resultBytes, options.fileName + '_modified.pdf');
    showSuccess('Pages deleted successfully!');

    // Analytics
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('delete-pages', processingTime, fileBytes.byteLength, 0, true);

  } catch (err) {
    hideProgress();
    showError(err.message || 'Page deletion failed');

    const processingTime = window.performance.now() - startTime;
    trackToolUsage('delete-pages', processingTime, fileBytes.byteLength, 0, false, err.message);
  }
}
