...
async function handleProcess(fileBytes, options) {
  ...
  try {
    showProgress(10, 'Encrypting PDF file...');
    ...
    const resultBytes = await runTask('protect', payload, {
    ...
    downloadFile(resultBytes, options.fileName + '_protected.pdf');
    showSuccess('PDF protected successfully!');

    // Analytics
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('protect', processingTime, fileBytes.byteLength, 0, true);
    ...
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('protect', processingTime, fileBytes.byteLength, 0, false, err.message);
  }
}
