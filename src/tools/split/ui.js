...
export function createUI(container, { locale = 'en', onProcess, onCancel }) {
    ...
    <div class="tool-container">
      ...
      <div id="workspace" class="workspace hidden">
        ...
        <div class="actions" style="display: flex; justify-content: center; margin-top: 2rem;">
          <button id="btn-apply" class="btn-action">✂... Split PDF</button>
        </div>
      </div>
    </div>
  `;
  ...
  container.querySelector('#btn-apply').addEventListener('click', () => {
    const rangesText = container.querySelector('#split-ranges').value.trim();
    if (!rangesText) {
      alert('Please enter ranges to split.');
      return;
    }
    ...
    if (ranges.length === 0) {
      alert('Invalid range format. Please use numbers, e.g. 1-3, 5');
      return;
    }
