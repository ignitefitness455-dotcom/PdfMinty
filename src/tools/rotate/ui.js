...
export function createUI(container, { locale = 'en', onProcess, onCancel }) {
  container.innerHTML = `
    <div class="tool-container">
      ...
      <div class="tool-header">
        <h1>Rotate PDF</h1>
        <p>Rotate pages in your PDF document to the correct orientation</p>
      </div>
      ...
      <div id="workspace" class="workspace hidden">
        ...
        <div class="settings-panel" style="grid-template-columns: 1fr; border: 1px solid rgba(255,255,255,0.05); border-radius: 8px;">
          <div class="setting-group full-width" style="margin-bottom: 1.5rem;">
            ...
            <select id="rotate-direction" class="select-input" style="width: 100%; border: 1px solid rgba(255,255,255,0.1); border-radius: 6px;">
          ...
          <div id="custom-degree-container" class="setting-group full-width" style="margin-bottom: 1.5rem; display: none;">
            ...
            <input type="number" id="custom-degree" class="text-input" value="45" style="border: 1px solid rgba(255,255,255,0.1); border-radius: 6px;" />
          </div>
        </div>
  ...
  const rotateSel = container.querySelector('#rotate-direction');
  const customDegContainer = container.querySelector('#custom-degree-container');

  rotateSel.addEventListener('change', () => {
    customDegContainer.style.display = rotateSel.value === 'custom' ? 'block' : 'none';
  });
  ...
  container.querySelector('#btn-apply').addEventListener('click', () => {
    const val = rotateSel.value;
    const degree = val === 'custom'
      ? (parseFloat(container.querySelector('#custom-degree').value) || 0)
      : (parseFloat(val) || 90);
