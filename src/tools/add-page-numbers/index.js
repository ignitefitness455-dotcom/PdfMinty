/**
 * src/tools/add-page-numbers/index.js
 */
...
async function handleProcess(fileBytes, options) {
  ...
  try {
    showProgress(10, 'Adding page numbers to PDF...');
    ...
    const resultBytes = await runTask('add-page-numbers', payload, {
    ...
    downloadFile(resultBytes, options.fileName + '_numbered.pdf');
    showSuccess('Page numbers added successfully!');

    // Analytics
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('add-page-numbers', processingTime, fileBytes.byteLength, 0, true);

  } catch (err) {
    hideProgress();
    showError(err.message || 'Adding page numbers failed');

    const processingTime = window.performance.now() - startTime;
    trackToolUsage('add-page-numbers', processingTime, fileBytes.byteLength, 0, false, err.message);
  }
}
