...
async function handleProcess(fileBytes, options) {
  ...
  try {
    showProgress(10, 'Decrypting PDF file...');
    ...
    const resultBytes = await runTask('unlock', payload, {
    ...
    downloadFile(resultBytes, options.fileName + '_unlocked.pdf');
    showSuccess('PDF unlocked successfully!');

    // Analytics
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('unlock', processingTime, fileBytes.byteLength, 0, true);
    ...
    const processingTime = window.performance.now() - startTime;
    trackToolUsage('unlock', processingTime, fileBytes.byteLength, 0, false, err.message);
  }
}
