...
export function createUI(container, { locale = 'en', onProcess, onCancel }) {
  container.innerHTML = `
    <div class="tool-container">
      ...
      <div class="tool-header">
        <h1>Reorder PDF</h1>
        <p>Change the sequence of pages of your PDF document</p>
      </div>
      ...
      <div id="workspace" class="workspace hidden">
        ...
        <div class="settings-panel" style="grid-template-columns: 1fr; border: 1px solid rgba(255,255,255,0.05); border-radius: 8px;">
          <div class="setting-group full-width" style="margin-bottom: 1.5rem;">
            ...
            <p style="font-size: 0.85rem; color: var(--muted); margin-top: 0.5rem;">Enter the page numbers in the sequence you want them to appear.</p>
          </div>
        </div>
  ...
  container.querySelector('#btn-apply').addEventListener('click', () => {
    const orderText = container.querySelector('#page-order').value.trim();
    if (!orderText) {
      alert('Please enter a new page order.');
      return;
    }

    let newOrder = [];
    for (let p of orderText.split(',')) {
      let pStr = p.trim();
      if (pStr.includes('-')) {
