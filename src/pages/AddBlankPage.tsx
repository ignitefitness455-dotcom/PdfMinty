...
export default function AddBlankPage() {
      ...
      try {
        ...
        try {
          ...
          if (err?.message?.includes("SECURED_LOCKED")) {
            ...
            setLoading(false);
            showToast(
              "🔒 Standard secured/locked PDF file detected. Blank page insertion is restricted. Use the Unlock tool first.",
              "error"
            );
            return;
          }
  ...
  const handleFilesSelected = (files: File[]) => {
    ...
    if (pdfs.length === 0) {
      showToast("Only PDF files are supported for adding pages.", "error");
      return;
    }
    ...
    try {
      ...
      worker.onmessage = (e: MessageEvent) => {
        ...
        if (success && bytes) {
          ...
          triggerDownload(bytes, "expanded_document.pdf", setCompletedResult);
      ...
      worker.onerror = (err) => {
        ...
        showToast("Worker error occurred during blank page insertion.", "error");
        setLoading(false);
        setProcessingProgress(null);
        worker.terminate();
      };
            ...
            <div className="space-y-6">
              <div className="text-left">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-450 mt-1 font-medium">
                  Insert clean blank white pages of custom dimenional sizing anywhere in between layouts programmatically.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3.5 pt-1">
                    <div className="space-y-1.5">
                      <label htmlFor="blank-size-select" className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                        Blank Page Standard Size
                      </label>
                      <select
                        id="blank-size-select"
                        value={blankPageSize}
                    ...
                    <div className="space-y-1.5">
                      <label htmlFor="blank-pos-select" className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                        Insertion Position Placement
                      </label>
                      <select
                        id="blank-pos-select"
                        value={blankPagePos}
                        onChange={(e) => setBlankPagePos(e.target.value)}
                        className="w-full text-xs font-bold px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 outline-none"
                      >
                        <option value="end">At the very end of document</option>
                        <option value="start">At the very beginning of files</option>
                        <option value="after">Directly after specific page</option>
                      </select>
                    </div>

                    {blankPagePos === "after" && (
                      <div className="space-y-1.5 animate-fadein">
                        <label htmlFor="blank-page-num" className="text-xs font-bold text-slate-750 dark:text-slate-300 block">
                          Target reference page index number (1 to {pdfPages.length || 1})
                        </label>
                        <input
                          id="blank-page-num"
                          type="number"
                          ...
                          value={blankPageAt}
                          onChange={(e) => setBlankPageAt(Math.max(1, parseInt(e.target.value) || 1))}
            ...
            {!loading && !completedResult && selectedFiles.length > 0 && (
              <div className="pt-6 border-t border-slate-100 dark:border-slate-850 mt-8">
                ...
                >
                  ...
                  <span>Insert Empty Page Frame</span>
                </button>
              </div>
            )}
          </div>
