...
export default function PageNumbersPage() {
      ...
      try {
        ...
        try {
          ...
          if (err?.message?.includes("SECURED_LOCKED")) {
            ...
            setLoading(false);
            showToast(
              "🔒 Standard secured/locked PDF file detected. Page numbering is restricted. Use the Unlock tool first.",
              "error"
            );
            return;
          }
  ...
  const handleFilesSelected = (files: File[]) => {
    ...
    if (pdfs.length === 0) {
      showToast("Only PDF files are supported for page numbering.", "error");
      return;
    }
    ...
    try {
      ...
      worker.onmessage = (e: MessageEvent) => {
        ...
        if (success && bytes) {
          ...
          triggerDownload(bytes, "numbered_document.pdf", setCompletedResult);
      ...
      worker.onerror = (err) => {
        ...
        showToast("Worker error occurred during page numbering.", "error");
        setLoading(false);
        setProcessingProgress(null);
        worker.terminate();
      };
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3.5 pt-1">
                    <div className="space-y-1.5">
                      <label htmlFor="number-format-select" className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                      ...
                      >
                        <option value="page-x-of-y">"Page X of Y" (Recommended)</option>
                        <option value="page-x">"Page X" (Simple number index)</option>
                        <option value="x">Only raw digits ("1", "2")</option>
                      </select>
                    </div>

                    <div className="space-y-1.5">
                      <label htmlFor="number-pos-select" className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                        Position Alignments
                      </label>
                      <select
                        id="number-pos-select"
                        value={pageNumberPosition}
                        onChange={(e) => setPageNumberPosition(e.target.value)}
                        className="w-full text-xs font-bold px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 outline-none"
                      >
                        <option value="bottom-center">Footer center (Standard)</option>
                        <option value="bottom-left">Footer bottom left</option>
                        <option value="bottom-right">Footer bottom right</option>
                        <option value="top-center">Header top center</option>
                        <option value="top-left">Header top left</option>
            ...
            {!loading && !completedResult && selectedFiles.length > 0 && (
              <div className="pt-6 border-t border-slate-100 dark:border-slate-850 mt-8">
                ...
                >
                  ...
                  <span>Stamping Numeric Sequences</span>
                </button>
              </div>
            )}
          </div>
