...
export const SplitPage: React.FC = () => {
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      setError(err?.message || 'An unexpected failure occurred while splitting the document. Verify bounds or password locks.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto" id="split_page_container">
      <SEO 
        title="Split PDF — Free Offline PDF Splitter" 
        description="Split PDF pages online into individual parts. Specify custom page intervals like '1-3, 5' and download segments instantly offline without server hops."
      />
      ...
      <div className="space-y-2">
        <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">Split PDF Document</h1>
        <p className="text-slate-500 text-sm">Partition a PDF file into custom constituent sections and download each part independently.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-4">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            {!selectedFile ? (
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between" id="loaded_split_file">
          ...
          <div className="bg-slate-100 p-4 rounded-xl flex items-start space-x-2 border border-slate-200 text-xs text-slate-600 leading-relaxed">
            ...
            <div>
              ...
              <ul className="list-disc pl-4 space-y-0.5">
                <li>Use single numbers to extract single pages: <code className="bg-white px-1 py-0.5 rounded border">5</code></li>
                <li>Use spans with en-dashes to extract page blocks: <code className="bg-white px-1 py-0.5 rounded border">1-3</code></li>
                <li>Separate segments with comma lists: <code className="bg-white px-1 py-0.5 rounded border">1-2, 4, 6-8</code> parses into 3 separate file outputs.</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between h-fit space-y-6">
          <div className="space-y-4">
            <h3 className="font-bold text-slate-900 border-b border-slate-100 pb-2">Split Directives</h3>
            
            <div className="space-y-2">
              <label htmlFor="range_directives" className="text-xs font-bold text-slate-600 uppercase tracking-wider block">Page ranges:</label>
              <input
                id="range_directives"
                type="text"
                value={ranges}
          ...
          <div className="space-y-3 pt-4 border-t border-slate-100">
            {error && (
              <div className="flex items-start space-x-1.5 text-xs text-rose-700 bg-rose-50 border border-rose-100 p-3 rounded-lg" id="split_error_box">
