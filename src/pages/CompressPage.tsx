    ...
    <div className="space-y-6 max-w-4xl mx-auto" id="compress_page_container">
      ...
      <div className="flex justify-center py-4" id="compress_main_area">
        {!selectedFile ? (
          ...
          <div className="w-full max-w-md bg-white border border-slate-200 rounded-[28px] p-6 md:p-7 shadow-xl space-y-5 animate-in fade-in zoom-in-95 duration-200 relative">
            {/* Modal Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                <div className="flex-shrink-0 w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-[0_4px_14px_rgba(5,150,105,0.3)]">
            ...
            {!complete ? (
              <>
                ...
                <div className="flex items-center justify-between p-3.5 bg-slate-50 border border-slate-100 rounded-2xl">
                  <div className="flex items-center space-x-2.5 min-w-0 flex-1">
                    <div className="flex-shrink-0 bg-emerald-600 text-white font-black text-[9px] px-1.5 py-1 rounded-md tracking-wider leading-none shadow-sm">
                      PDF
                    </div>
                  ...
                  <span className="text-xs font-extrabold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200/55 whitespace-nowrap ml-2 shadow-sm">
                    {(selectedFile.size / 1024 / 1024).toFixed(1)} MB
                  </span>
                </div>

                {/* Estimated Size Progress-like Bar */}
                <div className="flex items-center justify-between px-4 py-3 bg-emerald-50/10 border border-emerald-50 rounded-xl text-xs">
                  <span className="font-semibold text-slate-600">Estimated size:</span>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-4 rounded-2xl border-2 text-left transition-all flex items-start space-x-3.5 cursor-pointer ${
                          isSelected
                            ? 'border-emerald-500 dark:border-emerald-400 bg-emerald-50/10 ring-2 ring-emerald-500/15 dark:ring-emerald-400/25 shadow-sm shadow-emerald-600/10'
                            : 'border-slate-100 hover:border-slate-200 dark:hover:border-white/15 bg-white'
                        }`}
                      >
                        {/* Custom Radio Button */}
                        <div className="flex-shrink-0 mt-0.5">
                          {isSelected ? (
                            <div className="w-5 h-5 rounded-full border-2 border-emerald-500 dark:border-emerald-400 flex items-center justify-center bg-white">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline justify-between">
                            <span className={`font-black text-sm md:text-base ${isSelected ? 'text-emerald-900' : 'text-slate-800'}`}>
                              {profile.label}
                            </span>
                ...
                {/* Error Banner */}
                {error && (
                  <div className="flex items-start space-x-2 text-xs text-rose-700 bg-rose-50 border border-rose-100 dark:border-rose-400/20 p-3.5 rounded-xl">
                ...
                <button
                  ...
                  disabled={loading}
                  className={`w-full py-4 rounded-2xl font-black text-sm tracking-wide text-white flex items-center justify-center space-x-2 transition-all active:scale-[0.98] ${
                    loading
                      ? 'bg-slate-300 pointer-events-none'
                      : 'bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-700 hover:to-emerald-600 cursor-pointer hover:-translate-y-0.5 shadow-[0_8px_24px_rgba(5,150,105,0.25)] dark:shadow-[0_8px_30px_rgba(54,255,196,0.15)]'
                  }`}
                >
                  {loading ? (
              ...
              stats && (
                <div className="space-y-5 py-2 animate-in fade-in-50 duration-300" id="compression_stats">
                  <div className="mx-auto w-14 h-14 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex items-center justify-center shadow-[0_8px_24px_rgba(5,150,105,0.3)] dark:shadow-[0_8px_30px_rgba(54,255,196,0.2)]">
                  ...
                  <div className="grid grid-cols-3 gap-2.5">
                    ...
                    <div className="bg-emerald-50/10 p-3 rounded-xl border border-emerald-50 text-center">
                      <span className="text-[9px] uppercase tracking-wider text-slate-600 font-black block">
                        Optimized
                      </span>
                    ...
                    <div className="bg-gradient-to-br from-emerald-600 to-emerald-500 p-3 rounded-xl text-center shadow-[0_4px_16px_rgba(5,150,105,0.25)]">
                      <span className="text-[9px] uppercase tracking-wider text-emerald-100 font-black block">
                        Saved
                      </span>
                      <span className="font-extrabold text-white text-xs md:text-sm block mt-0.5">
                        {stats.ratio}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-2">
                    <button
                      ...
                      className="w-full py-3.5 px-4 bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white font-black text-sm rounded-xl flex items-center justify-center space-x-1.5 transition-all cursor-pointer shadow-[0_8px_20px_rgba(5,150,105,0.25)] dark:shadow-[0_8px_24px_rgba(54,255,196,0.12)]"
                    >
