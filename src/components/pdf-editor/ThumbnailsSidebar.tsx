...
export const ThumbnailsSidebar: React.FC<ThumbnailsSidebarProps> = ({
  ...
  return (
    <aside
      className="w-36 sm:w-48 bg-slate-100/90 border-r border-slate-200/80 flex flex-col shrink-0 h-full select-none z-10 transition-all"
      id="pdf_editor_thumbnails_sidebar"
    >
      {/* Sidebar Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 bg-white/70">
        <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">
          Pages ({pages.length})
        </span>
        <button
          onClick={onClose}
          className="p-1 text-slate-400 hover:text-slate-700 rounded-md hover:bg-slate-200/60 transition-colors md:hidden"
