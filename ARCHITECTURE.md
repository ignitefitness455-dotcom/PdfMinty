...
sequenceDiagram
    ...
    User->>Main: Clicks "Apply" (e.g., Compress, Merge, Crop/Resize)
    ...
    Worker->>Worker: Processes PDF (pdf-lib, pdfjs-dist)
...
3.  **pdf-worker.js**: All heavy lifting using `pdf-lib` and `pdfjs-dist` happens in the Web Worker. This guarantees that complex processes (like rendering, compressing, splitting, cropping, resizing) do not block or freeze the main browser UI thread.
